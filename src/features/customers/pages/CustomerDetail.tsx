...
const CustomerDetail = () => {
    ...
    const orderColumns: ColumnDef<Order>[] = [
        ...
        },
        {
            ...
            header: 'Amount',
            cell: (info) => <span className="font-bold text-indigo-600">₹{(info.getValue() as number).toFixed(2)}</span>
        },
        {
            accessorKey: 'status',
            header: 'Status',
        ...
        <div className="p-4 space-y-4 lg:p-8 mx-auto">
            ...
            <div className="flex items-center justify-between bg-zinc-100 dark:bg-zinc-800 p-4 rounded-3xl text-white shadow-xl">
                <div className="flex items-center gap-6">
                    <div className="w-16 h-16 bg-zinc-200 dark:bg-zinc-700 rounded-2xl flex items-center justify-center text-3xl text-zinc-600 dark:text-zinc-300 font-bold">
                        {customer.full_name?.charAt(0) || 'G'}
                    </div>
                    <div>
                        <h1 className="text-2xl text-zinc-600 dark:text-zinc-200 font-bold uppercase tracking-tight">{customer.full_name || 'Guest Customer'}</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <Card className="bg-gradient-to-br from-indigo-600 to-indigo-800 text-white border-0">
                    <p className="text-indigo-100 text-xs font-bold uppercase tracking-wider mb-2">Lifetime Orders</p>
                    <h2 className="text-3xl text-zinc-200 dark:text-zinc-200 font-bold">{stats?.total_orders}</h2>
                ...
                <Card className="relative overflow-hidden group">
                    ...
                    <p className="text-zinc-500 text-xs font-bold uppercase tracking-wider mb-2">Orders This Month</p>
                    <h2 className="text-3xl text-zinc-600 dark:text-zinc-200 font-bold text-zinc-900 dark:text-white">{stats?.current_month_orders}</h2>
                    <div className="flex items-center gap-1 mt-1">
                        <span className={`text-[10px] font-bold ${(stats?.current_month_orders || 0) >= (stats?.last_month_orders || 0) ? 'text-emerald-500' : 'text-red-500'}`}>
                ...
                <Card className="relative overflow-hidden group">
                    ...
                    <p className="text-zinc-500 text-xs font-bold uppercase tracking-wider mb-2">Avg Order Value (C.M.)</p>
                    <h2 className="text-3xl text-zinc-600 dark:text-zinc-200 font-bold text-indigo-600 dark:text-indigo-400">₹{stats?.current_month_aov?.toFixed(2)}</h2>
                ...
                <Card className="relative overflow-hidden group bg-zinc-50 dark:bg-zinc-800/50">
                    <p className="text-zinc-500 text-xs font-bold uppercase tracking-wider mb-2">Last Month AOV</p>
                    <h2 className="text-2xl text-zinc-600 dark:text-zinc-200 font-bold text-zinc-900 dark:text-white">₹{stats?.last_month_aov?.toFixed(2)}</h2>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                ...
                <Card className="lg:col-span-1 p-0 overflow-hidden h-fit">
                    <div className="p-6 border-b dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-800/50">
                        <h4 className="text-sm text-zinc-600 dark:text-zinc-200 font-bold uppercase tracking-widest text-zinc-900 dark:text-white flex items-center gap-2">
                        ...
                        {stats?.top_products.length ? stats.top_products.map((product, idx) => (
                            <div key={product.menu_item_id} className="p-4 flex items-center justify-between hover:bg-zinc-50 dark:hover:bg-zinc-800/30 transition-colors">
                                <div className="flex items-center gap-4">
                                    <span className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold ${idx < 3 ? 'bg-indigo-600 text-white' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-400'}`}>
                                        {idx + 1}
                                    </span>
                                ...
                                <div className="text-right">
                                    <span className="text-xs text-zinc-600 dark:text-zinc-200 font-bold text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/20 px-2 py-1 rounded">
                ...
                <div className="lg:col-span-2 space-y-6">
                    <Card className="p-0 overflow-hidden">
                        <div className="p-6 border-b dark:border-zinc-800">
                            <h4 className="text-sm text-zinc-600 dark:text-zinc-200 font-bold uppercase tracking-widest text-zinc-900 dark:text-white">Order History</h4>
                        </div>
                        <DataTable
                            data={orders || []}
