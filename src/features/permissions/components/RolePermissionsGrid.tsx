...
export const RolePermissionsGrid: React.FC = () => {
    ...
    return (
        <div className="space-y-6">
                ...
                {EDITABLE_ROLES.map((r) => {
                    ...
                    return (
                        <button
                            ...
                            className={`flex flex-col items-start p-4 rounded-2xl border text-left transition-all ${isSelected
                                ? 'bg-indigo-50/70 dark:bg-indigo-950/40 border-indigo-500/50 shadow-sm ring-1 ring-indigo-500/30'
                                : 'bg-white dark:bg-mauve-900 border-mauve-200 dark:border-mauve-800 hover:border-zinc-300 dark:hover:border-zinc-700'
                                }`}
                        >
                            <div className="flex items-center justify-between w-full mb-2">
                                <div
                                    className={`w-9 h-9 rounded-xl flex items-center justify-center text-lg ${isSelected
                                        ? 'bg-indigo-600 text-white'
                                        : 'bg-neutral-100 dark:bg-neutral-800 text-zinc-600 dark:text-zinc-400'
                                        }`}
                                >
                                    <i className={r.icon} />
                                </div>
                            ...
                            <span className="font-bold text-sm text-neutral-900 dark:text-white">{r.label}</span>
                            <span className="text-xs text-neutral-500 dark:text-neutral-400">Configure access rules</span>
                        </button>
                    );
                })}
            </div>
            ...
            <div className="bg-white dark:bg-mauve-900 border border-mauve-200 dark:border-mauve-800 rounded-2xl shadow-sm overflow-hidden">
                {/* Header bar */}
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-5 sm:p-6 border-b border-zinc-100 dark:border-mauve-800 bg-neutral-50/50 dark:bg-mauve-900/50">
                    <div>
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                        <button
                            ...
                            className="px-3 py-1.5 text-xs font-semibold rounded-lg bg-neutral-100 dark:bg-neutral-800 text-zinc-700 dark:text-zinc-300 hover:bg-neutral-200 dark:hover:bg-neutral-700 transition-colors"
                        >
                            Select All
                        </button>
                        <button
                ...
                <div className="p-5 sm:p-6 space-y-8">
                    {Object.entries(groupedModules).map(([category, items]) => (
                        <div key={category} className="space-y-3">
                            <div className="flex items-center gap-2 border-b border-zinc-100 dark:border-mauve-800 pb-2">
                                <span className="text-xs font-bold uppercase tracking-wider text-zinc-400 dark:text-zinc-500">
                                    {category}
                                </span>
                                <span className="text-[10px] px-2 py-0.5 rounded-full bg-neutral-100 dark:bg-neutral-800 text-zinc-500 font-semibold">
                                ...
                                {items.map((item) => {
                                    ...
                                    return (
                                        <div
                                            ...
                                            className={`flex items-start justify-between p-3.5 rounded-xl border cursor-pointer select-none transition-all ${isEnabled
                                                ? 'bg-indigo-50/30 dark:bg-indigo-950/20 border-indigo-200 dark:border-indigo-900/60'
                                                : 'bg-neutral-50/50 dark:bg-neutral-800/30 border-mauve-200/80 dark:border-mauve-800/80 hover:bg-neutral-100/60 dark:hover:bg-neutral-800/60'
                                                }`}
                                        >
                                            <div className="pr-3 space-y-0.5">
                                                <div className="flex items-center gap-1.5">
                                                    <span className="text-xs font-bold text-neutral-900 dark:text-zinc-100">
                                                        {item.name}
                                                    </span>
                                                </div>
                                            ...
                                            {/* Toggle switch */}
                                            <div
                                                className={`relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out ${isEnabled ? 'bg-indigo-600' : 'bg-neutral-300 dark:bg-neutral-700'
                                                    }`}
                                            >
                                                <span
