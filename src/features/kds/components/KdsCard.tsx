...
export const KdsCard: React.FC<KdsCardProps> = ({ order, onViewDetails }) => {
    ...
    const getStatusColor = (status: string) => {
        switch (status) {
            ...
            case 'SERVED': return 'bg-neutral-100 text-zinc-600 dark:bg-neutral-800 dark:text-zinc-400';
            default: return 'bg-neutral-100 text-zinc-600';
        }
    };
    ...
    return (
        <div className="bg-white dark:bg-mauve-900 rounded-xl border border-zinc-100 dark:border-mauve-800 shadow-sm overflow-hidden flex flex-col h-full ring-1 ring-zinc-950/5">
            {/* Header */}
            <div className="p-4 border-b border-zinc-100 dark:border-mauve-800 bg-neutral-50/50 dark:bg-neutral-800/30 flex justify-between items-start">
                <div>
                ...
                {order.items.filter(item => !item.order_deal_id).map((item) => (
                    <div key={item.id} className="flex justify-between items-center gap-2 group">
                        <div className="flex-grow min-w-0">
                            ...
                            {item.variant_name && item.variant_name !== 'Default' && (
                                <span className="text-[10px] text-neutral-500 dark:text-neutral-400 block font-semibold truncate">
                                    {item.variant_name}
                                </span>
                            )}
                        </div>
            ...
            {/* Footer Actions */}
            {action && (
                <div className="p-3 border-t border-zinc-100 dark:border-mauve-800 bg-neutral-50/30 dark:bg-mauve-900/30">
                    <Button
